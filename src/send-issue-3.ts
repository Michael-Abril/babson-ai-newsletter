...
async function main() {
  console.log(`Sending Issue #3 in ${mode} mode...`);

  // Load generated content
  const htmlPath = "output/newsletter-2026-03-16.html";
  const jsonPath = "output/newsletter-2026-03-16.json";

  if (!existsSync(htmlPath) || !existsSync(jsonPath)) {
    throw new Error("Newsletter files not found. Run npm run multi-agent:preview first.");
  }
  ...
  switch (mode) {
    case "preview": {
      ...
      console.log(`Open output/newsletter-2026-03-16.html in browser to view.`);
      break;
    }
    case "test": {
